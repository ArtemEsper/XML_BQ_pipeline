...
def main():
    ...
    parser.add_option("-s", "--single_trans", dest="single_trans",
                      help="If true, will enable one transaction per file, cannot have transaction statements in the template, if so")
    ...
    parser.add_option("-z", "--recurse", dest="recurse",
                      help="If true and a directory is set, the parser will search subdirectories for XML files to parse as well, ignored for single file parse")
    ...
    for filename in filelist:
        ...
        if mode == "file":
            if output_file is not None:
                ...
                outputtarget = os.path.join(output_dir, os.path.split(filename)[1][:-4] + "-queries.sql")
        else:
            outputtarget = os.path.join(output_dir, os.path.split(filename)[1][:-4] + "-queries.sql")
                ...
                if elem.tag == "%s%s" % (namespace, rec_tag):
                    ...
                    if valuepath in value_dict:
                        if root.text is not None:
                            tableList.AddCol(value_dict[valuepath].split(":", 1)[0],
                                             value_dict[valuepath].split(":", 1)[1], str(root.text))
...
def ParseNode(node, path, tableList, last_opened, statementList):
    ...
    if valuepath in value_dict:
        if node.text is not None:
            tableList.AddCol(value_dict[valuepath].split(":", 1)[0], value_dict[valuepath].split(":", 1)[1],
                             str(node.text))
